...
<title>查看报销申请</title>
 ...
 <%
	ArrayList<Trans> trans =(ArrayList<Trans>)request.getSession().getAttribute("trans");
 	...
 	int id= 1;
 %>
 <a href="" onclick="back()">报销申请单《《</a><a>报销单详情</a>
<div class="container" >
   <div class="row" >
      <div class="col-xs-12" > 
      <center > 
...
<center>
 ...
 <tr><td>单位</td><td>XX公司</td>
  ...
  <tr><td class="col-sm-3">序号</td><td class="col-sm-3">项目</td><td class="col-sm-3">金额</td><td class="col-sm-3">票据张数</td></tr>
  <s:iterator value="#request.other_expenses" var="Other_expense" status="st">
  <tr onclick="div(this)" style="height:50px" class="first">
  ...
  <s:iterator value="#request.trans" var="Trans" status="st1">
   <tr>
   <td style="padding:10px;"><s:property value="#Trans.trans_start_time"/></td>
   <td style="padding:10px;"><s:property value="#Trans.trans_end_time"/></td>
   <td style="padding:10px;"><s:property value="#Trans.trans_start_address"/></td>
   <td style="padding:10px;"><s:property value="#Trans.trans_end_address"/></td>
   <td style="padding:10px;"><s:property value="#Trans.trans_money"/></td>
   <input name='txtTRLastIndex' type='hidden' id='txtTRLastIndex' value="1" />
   </tr>
   </s:iterator>
   </table><font face="幼圆" size="5">报销详情记录
   <textarea rows="5" cols="90" name="detail" id="detail" required="required"></textarea>	<br>
   	总报销金额：<input type="text" name="money" required="required" />
   <input type="button" name="button" id="button" class="btn btn-primary"  onclick="Isconfirm()" value="确认报销"></font>
